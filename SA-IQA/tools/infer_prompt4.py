...
def get_default_test_jsonl(data_root: str, dimension: str) -> str:
    ...
    return os.path.join(
        data_root,
        "annotations",
        f"{dimension}_{subset}_test_prompt4.jsonl",
    )
...
def parse_args():
    parser = argparse.ArgumentParser(description="Inference and evaluation script for SA-IQA prompt4.")
...
def main():
    ...
    message_content = PROMPT4[args.dimension]
    ...
    result_stem = f"{args.dimension}_prompt4_{model_name}"
